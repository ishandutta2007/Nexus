...
// Per-process secret proving x-nx-real-ip was stamped below rather than sent by the client.
// A bare `next start` / `next dev` never loads this file, so it cannot produce a matching
// header even though the env var is inherited by child processes. Named like x-nx-cli-token
// so the request-detail header sanitizer redacts it too.
const PEER_TOKEN = crypto.randomBytes(24).toString("hex");
process.env.NEXUS_PEER_TOKEN = PEER_TOKEN;
...
http.createServer = (...args) => {
  ...
  const wrapped = (req, res) => {
    ...
    delete req.headers["x-nx-real-ip"];
    delete req.headers["x-forwarded-for"];
    delete req.headers["x-nx-via-proxy"];
    delete req.headers["x-nx-peer-token"];
    req.headers["x-nx-real-ip"] = ip;
    req.headers["x-nx-peer-token"] = PEER_TOKEN;
    if (viaProxy) req.headers["x-nx-via-proxy"] = "1";
