...
async function buildCodexHeader() {
  ...
  const { installed, hasNexus, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasNexus, settings } = result.data;
  ...
  // Extract Nexus custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:Nexus-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract Nexus provider config
  const provider = settings?.models?.providers?.["nexus"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("nexus/") ? primary.replace("nexus/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasNexus, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use Nexus\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
