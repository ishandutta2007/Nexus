...
const cliAppDir = process.env.NEXUS_CLI_APP_DIR || path.join(cliDir, "app");
...
function resolveStandaloneBuild(appDir, buildDistDir) {
  ...
  // NEXT_TRACING_ROOT_MODE=workspace, e.g. standalone/nexus/server.js.
...
function buildCliPackage() {
  console.log("📦 Building Nexus CLI package with Next.js...\n");
  ...
  // Strip better-sqlite3 (native) — it lives in ~/.nexus/runtime to avoid
  ...
  if (fs.existsSync(betterDir)) {
    ...
    console.log("✅ Stripped better-sqlite3 (lives in ~/.nexus/runtime)");
  }
  console.log("");
