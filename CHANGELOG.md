...
## Security
- **Real IP**: `x-nx-real-ip` and the Host fallback were trusted from
  client-controlled headers whenever `custom-server.js` was not in the request
  path (`npm run start`, `start:bun`), letting a remote caller pose as local to
  skip API key auth and reach `LOCAL_ONLY_PATHS` (`/api/mcp/*`,
  `/api/tunnel/enable`, `/api/auth/reset-password`). The server now stamps a
  per-process `x-nx-peer-token` on every request it sanitizes and only trusts
  `x-nx-real-ip` behind it — falling back to Host in development and failing
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point Nexus at your own
...
- **RTK**: `X-Nexus-Token-Saver` header to bypass token savers per request
...
## Docs
- README: add Indonesian Nexus tutorial video (#1709)
...
- Proxy: raise Next client body limit to 128MB via `NEXUS_PROXY_CLIENT_MAX_BODY_SIZE` (#1529, #1572)
