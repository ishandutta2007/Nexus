<div align="center">
  ...
  **Hubungkan semua tool AI coding (Claude Code, Cursor, Antigravity, Copilot, Codex, Gemini, OpenCode, Cline, OpenClaw...) ke 40+ provider AI dan 100+ model.**
  ...
  [🚀 Mulai Cepat](#-mulai-cepat) • [💡 Fitur](#-fitur-utama) • [📖 Setup](#-panduan-setup) • [🌐 Website](https://nexus.com)
...
Paket repo ini bersifat privat (`nexus-app`), jadi menjalankan dari source/Docker adalah jalur yang diharapkan untuk pengembangan lokal.
...
### 📺 Panduan Setup Lengkap - Nexus + Claude Code Gratis

[![Nexus + Claude Code Setup](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Tonton tutorial langkah demi langkah:**
- ✅ Install dan setup Nexus
...
Nexus bekerja mulus dengan semua tool AI coding utama:
...
- Tool CLI mengirim dalam format OpenAI → Nexus mengonversi → provider menerima dalam format nativenya
...
> Nexus sendiri **tidak menagih apa pun**. Kamu hanya membayar langsung ke provider jika memakai layanan berbayar.
>
...
>
> Anggap saja ini "pelacak penghematan" yang menunjukkan berapa banyak yang kamu hemat lewat model gratis dan routing Nexus!
...
### 📊 Tentang Biaya dan Penagihan Nexus

**Fakta soal penagihan Nexus:**

✅ **Software Nexus = gratis selamanya** (open source, tanpa tagihan)
...
❌ **Nexus tidak mengirim invoice** atau menagih kartumu
...
- **Provider murah** (GLM, MiniMax): bayar langsung, Nexus hanya melakukan routing
- **Provider gratis** (iFlow, Kiro, Qwen): benar-benar gratis selamanya, tanpa biaya tersembunyi
- **Nexus**: tidak menagih apa pun
...
Dashboard melacak pemakaian token dan menampilkan **estimasi biaya** seandainya kamu memakai API berbayar secara langsung. Ini **bukan tagihan nyata**, melainkan acuan untuk melihat berapa banyak yang kamu hemat dengan memakai model gratis atau langganan yang sudah ada lewat Nexus.
...
<details>
<summary><b>💳 Apakah Nexus menagih saya?</b></summary>

**Tidak.** Nexus adalah software open source gratis yang berjalan di komputermu sendiri. Tidak ada penagihan sama sekali.

**Kamu membayar ke:**
- ✅ **Provider langganan** (Claude Code $20/bulan, Codex $20-200/bulan) → bayar langsung di website masing-masing
- ✅ **Provider murah** (GLM, MiniMax) → bayar langsung, Nexus hanya me-routing request
- ❌ **Nexus sendiri** → **tidak menagih apa pun**

Nexus adalah proxy/router lokal. Ia tidak menyimpan informasi kartu kredit, tidak bisa mengirim invoice, dan tidak punya sistem penagihan. Sepenuhnya software gratis.
...
Nexus hanya me-routing request — tidak ada "jebakan" atau tagihan di kemudian hari. Layanannya memang gratis, dan Nexus membuatnya lebih mudah dipakai dengan dukungan fallback.

**Catatan:** beberapa provider langganan (Antigravity, GitHub Copilot) punya masa preview gratis dan bisa jadi berbayar nanti, tetapi hal itu diumumkan secara jelas oleh provider tersebut, bukan oleh Nexus.
...
3. **Gunakan provider langganan paling akhir:**
   ...
   - Nexus memaksimalkan nilainya lewat pelacakan kuota
...
Smart fallback Nexus mencegah tagihan tak terduga:
...
**Kamu yang pegang kendali:** atur batas pengeluaran per provider di dashboard, dan Nexus akan mematuhinya.
...
**Tips pro:** pakai Opus untuk tugas kompleks, Sonnet kalau mengutamakan kecepatan. Nexus melacak kuota per model!
...
```bash
npm install -g nexus
PORT=20128 HOSTNAME=0.0.0.0 BASE_URL=https://your-domain.com nexus
```
...
MIT License — lihat [LICENSE](https://github.com/decolua/nexus/blob/main/LICENSE) untuk detailnya.

---

<div align="center">

**Kalau Nexus membantumu, kasih ⭐ di [GitHub](https://github.com/decolua/nexus)!**

[🌐 Website](https://nexus.com) • [📦 npm](https://www.npmjs.com/package/nexus) • [🐛 Laporkan Bug](https://github.com/decolua/nexus/issues)
