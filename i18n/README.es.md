<div align="center">
  <img src="../images/nexus.png?1" alt="Panel de control de Nexus" width="800"/>
  
  # Nexus - Enrutador de IA GRATUITO y ahorrador de tokens
...
[🚀 Inicio rápido](#-inicio-rápido) • [💡 Características](#-características-principales) • [📖 Configuración](#-guía-de-instalación) • [🌐 Sitio web](https://nexus.com)
...
El paquete de este repositorio es privado (`nexus-app`), por lo que ejecutar desde el código fuente/Docker es la ruta de desarrollo local prevista.
...
> 🎬 **¿Has hecho un video sobre Nexus?** Envía una [Pull Request](https://github.com/decolua/nexus/pulls) añadiendo tu video a esta sección — ¡lo fusionaremos!

---

## 🛠️ Herramientas CLI compatibles

Nexus funciona a la perfección con todas las principales herramientas de código con IA:
...
Headroom es opcional y se ejecuta por separado. Nexus llama al endpoint local `/v1/compress` de Headroom y luego mantiene el enrutamiento, la reserva, la autenticación y el seguimiento de uso normales:

```
Client → Nexus → Headroom /v1/compress → Nexus → provider
```
...
Si Headroom está caído o devuelve un error, Nexus falla de forma segura y envía la solicitud original.
...
- Tu herramienta CLI envía el formato OpenAI → Nexus lo traduce → El proveedor recibe el formato nativo
...
> Nexus por sí mismo **nunca te cobra** nada. Solo pagas a los proveedores directamente (si usas servicios de pago).
>
...
### 📊 Cómo entender los costes y la facturación de Nexus

**Realidad de la facturación de Nexus:**

✅ **El software de Nexus = GRATUITO para siempre** (código abierto, nunca cobra)  
...
❌ **Nexus nunca envía facturas** ni carga tu tarjeta
...
- **Proveedores económicos** (GLM, MiniMax): Págales directamente, Nexus solo enruta
- **Proveedores GRATUITOS** (iFlow, Kiro, Qwen): Realmente gratuitos para siempre, sin cargos ocultos
- **Nexus**: Nunca cobra nada, jamás
...
El panel realiza el seguimiento de tu uso de tokens y muestra **costes estimados** como si estuvieras usando APIs de pago directamente. Esto **no es facturación real** — es una referencia para mostrar cuánto estás ahorrando al usar modelos gratuitos o suscripciones existentes a través de Nexus.
...
<details>
<summary><b>💳 ¿Nexus me cobrará algo?</b></summary>

**No.** Nexus es un software gratuito y de código abierto que se ejecuta en tu propio ordenador. Nunca te cobra nada.

**Solo pagas:**

- ✅ **Proveedores de suscripción** (Claude Code $20/mes, Codex $20-200/mes) → Págales directamente en sus sitios web
- ✅ **Proveedores económicos** (GLM, MiniMax) → Págales directamente, Nexus solo enruta tus solicitudes
- ❌ **El propio Nexus** → **Nunca cobra nada, jamás**

Nexus es un proxy/enrutador local. No tiene tu tarjeta de crédito, no puede enviar facturas y no tiene sistema de facturación. Es software totalmente gratuito.
...
Nexus solo enruta tus solicitudes hacia ellos — no hay "trampa" ni facturación futura. Son servicios realmente gratuitos y Nexus los hace fáciles de usar con soporte de reserva.
...
3. **Usa los proveedores de suscripción al final:**
   ...
   - Nexus te ayuda a maximizar su valor mediante el seguimiento de cuota
...
La reserva inteligente de Nexus evita cargos sorpresa:
...
**Tú tienes el control:** Establece límites de gasto por proveedor en el panel y Nexus los respeta.
...
**Consejo profesional:** Usa Opus para tareas complejas y Sonnet para velocidad. ¡Nexus realiza el seguimiento de la cuota por modelo!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [desde el panel de nexus]
...
```json
{
  ...
  "anthropic_api_key": "tu-clave-api-de-nexus"
}
```
...
export OPENAI_API_KEY="tu-clave-api-de-nexus"
...
> **Nota:** OpenClaw solo funciona con Nexus local. Usa `127.0.0.1` en lugar de `localhost` para evitar problemas de resolución IPv6.
...
docker pull decolua/nexus:latest   # actualiza a la última versión
```

**Persistencia de datos:** `$HOME/.nexus/db/data.sqlite` en el host ↔ `/app/data/db/data.sqlite` en el contenedor.

### Variables de entorno

| Variable                                             | Por defecto                                | Descripción                                                                          |
| ---------------------------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------ |
| `JWT_SECRET`                                         | Generado automáticamente (`~/.nexus/jwt-secret`) | Secreto de firma JWT para la cookie de autenticación del panel (sobreescríbelo para compartir entre instancias) |
| `INITIAL_PASSWORD`                                   | `123456`                                   | Contraseña del primer inicio de sesión cuando no existe un hash guardado             |
| `DATA_DIR`                                           | `~/.nexus`                               | Ubicación principal de datos de la app (SQLite en `$DATA_DIR/db/data.sqlite`)        |
...
| `CLOUD_URL`                                          | `https://nexus.com`                      | URL base del endpoint de sincronización en la nube del lado del servidor             |
...
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://nexus.com`                      | URL de nube pública/compatible (prefiere `CLOUD_URL` para el runtime del servidor)   |
...
- Tanto `${DATA_DIR}` como `~/.nexus` resuelven a la misma ubicación en un contenedor de Docker — el enlace simbólico `/root/.nexus -> /app/data` se crea en tiempo de compilación.
...
- Nexus lo renueva automáticamente
...
- **Sitio web**: [nexus.com](https://nexus.com)
- **GitHub**: [github.com/decolua/nexus](https://github.com/decolua/nexus)
- **Problemas**: [github.com/decolua/nexus/issues](https://github.com/decolua/nexus/issues)

---

## 👥 Colaboradores

¡Gracias a todos los colaboradores que ayudaron a mejorar Nexus!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork de Nexus en TypeScript con todas las funciones. Añade más de 36 proveedores, reserva automática de 4 niveles, APIs multimodales (imágenes, embeddings, audio, TTS), interruptor de circuitos, caché semántica, evaluaciones de LLM y un panel pulido. Más de 368 pruebas unitarias. Disponible a través de npm y Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — ahorrador de tokens en Rust. Nexus porta su pipeline de compresión a JS → **−20-40% de tokens de entrada** en cada solicitud.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) por **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. Nexus adapta su prompt → **−65% de tokens de salida**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) por **[@DietrichGebert](https://github.com/DietrichGebert)** — habilidad del _"dev sénior perezoso"_. Nexus inyecta su jerarquía primero-YAGNI → **menos tokens, menos código, diffs más cortos**.

Un enorme agradecimiento a estos autores — sin su trabajo, las funciones de ahorro de tokens de Nexus no existirían. ¡Dales una ⭐ en GitHub!
