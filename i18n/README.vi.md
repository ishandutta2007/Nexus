<div align="center">
  <img src="../images/nexus.png?1" alt="Bảng điều khiển Nexus" width="800"/>
  ...
  **Kết nối tất cả công cụ AI Code (Claude Code, Codex, Cursor, Cline, Copilot, Antigravity...) tới 40+ Nhà cung cấp AI & 100+ Mô hình.**
  ...
  [![License](https://github.com/decolua/nexus/blob/main/LICENSE)](https://github.com/decolua/nexus/blob/main/LICENSE)
  
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://nexus.com)
</div>
...
Gói kho lưu trữ này là riêng tư (`nexus-app`), vì vậy việc chạy từ nguồn/Docker là cách phát triển cục bộ mặc định.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - Nexus + Claude Code MIỄN PHÍ
  
[![Thiết lập Nexus + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập Nexus
...
Nexus hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → Nexus dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân Nexus **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của Nexus

**Thực tế Thanh toán Nexus:**

✅ **Phần mềm Nexus = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **Nexus không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, Nexus chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị Nexus tính phí không?</b></summary>

**Không.** Nexus là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, Nexus chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân Nexus** → **Không bao giờ tính phí bất cứ thứ gì, ever**

Nexus là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
Nexus chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và Nexus giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải Nexus.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - Nexus giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của Nexus ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và Nexus sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. Nexus theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển nexus]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với Nexus cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| Biến | Mặc định | Mô tả |
|----------|---------|-------------|
| `JWT_SECRET` | Tự động sinh (`~/.nexus/jwt-secret`) | Bí mật ký JWT cho cookie xác thực bảng điều khiển (đặt để chia sẻ giữa nhiều instance) |
| `INITIAL_PASSWORD` | `123456` | Mật khẩu đăng nhập đầu tiên khi không có hash đã lưu tồn tại |
| `DATA_DIR` | `~/.nexus` |ị trí cơ sở dữ liệu ứng dụng chính (`db.json`) |
...
| `CLOUD_URL` | `https://nexus.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://nexus.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Lịch sử sử dụng và log: `~/.nexus/usage.json` và `~/.nexus/log.txt`, được quản lý bởi `src/lib/usageDb.js`.
- request/translator tùy chọn: `<repo>/logs/...` khi `ENABLE_REQUEST_LOGS=true`.
- Lưu trữ sử dụng hiện tại tuân theo logic đường dẫn `~/.nexus` và độc lập với `DATA_DIR`.
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://nexus.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://nexus.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime Nexus cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://nexus.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://nexus.com/v1/chat/completions`):
...
5. `handleChatCore` (`engine/handlers/chatCore.js`) phát hiện định dạng và dịch chuyển yêu cầu.
...
- **GitHub**: [github.com/decolua/9](https://github.com/decolua/nexus)
...
Cảm ơn tất cả những người đã đóng góp giúp Nexus tốt hơn!
...
[![ Chart](https://starchart.cc/decolua/nexus.svg?variant=adaptive)](https://starchart.cc/decolua/nexus)
