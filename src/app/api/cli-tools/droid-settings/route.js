...
// Check if settings has Nexus customModels
const hasNexusConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Nexus"));
};
...
// POST - Update Nexus customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Nexus configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Nexus"));
...
// DELETE - Remove Nexus customModels only (keep other settings)
