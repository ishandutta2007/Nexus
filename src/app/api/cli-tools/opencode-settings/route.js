...
const hasNexusConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["nexus"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["nexus"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasNexus: hasNexusConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("nexus/") ? config.model.replace(/^nexus\//, "") : null,
...
// POST - Apply Nexus as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_nexus";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing nexus provider entry and its models
    const existingProvider = config.provider["nexus"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["nexus"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `nexus/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `nexus/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("nexus/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Nexus provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["nexus"]?.models) {
      delete config.provider["nexus"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["nexus"].models).length === 0) {
        delete config.provider["nexus"];
        if (config.model?.startsWith("nexus/")) delete config.model;
      } else if (config.model === `nexus/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["nexus"].models);
        config.model = `nexus/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire nexus provider
      if (config.provider) delete config.provider["nexus"];
      if (config.model?.startsWith("nexus/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("nexus/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "Nexus settings removed from OpenCode",
    });
  } catch (error) {
