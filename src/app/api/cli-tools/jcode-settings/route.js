...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-nexus.env");
};
...
const hasNexusConfig = (config) => {
  ...
  if (providers["nexus"]) return true;
...
export async function GET() {
  ...
  const hasNexus = hasNexusConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_NEXUS_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile nexus",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["nexus"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_NEXUS_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "nexus configuration removed from jcode",
    });
  } catch (error) {
