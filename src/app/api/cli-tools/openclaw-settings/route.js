...
// Check if settings has Nexus config
const hasNexusConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["nexus"];
};

// Read per-agent models.json and return current model id (without "nexus/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["nexus"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasNexus: hasNexusConfig(settings),
...
// POST - Update Nexus settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `nexus/${model}`;

    // Remove all old nexus/* entries from agents.defaults.models
    ...
    // Add fresh nexus models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`nexus/${m}`] = {};
    });

    // Remove old nexus model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("nexus/")) {
    ...
    // Update models.providers.nexus with all models
    settings.models.providers["nexus"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `nexus/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Nexus settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Nexus from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["nexus"];
    ...
    // Remove nexus models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("nexus/"));
    ...
    // Reset agents.defaults.model.primary if it uses nexus
    if (settings.agents?.defaults?.model?.primary?.startsWith("nexus/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Nexus settings removed successfully",
    });
  } catch (error) {
