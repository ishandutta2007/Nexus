...
const hasNexusConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Nexus");
};

const getNexusEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Nexus") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getNexusEntry(config);
...
// POST - Apply Nexus config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Nexus entry or append
    const idx = config.findIndex((e) => e.name === "Nexus");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Nexus entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Nexus");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Nexus removed from Copilot config",
    });
  } catch (error) {
