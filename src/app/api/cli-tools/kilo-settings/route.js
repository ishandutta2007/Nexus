...
const hasNexusConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["nexus"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("nexus");
};
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Nexus", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Nexus settings removed from Kilo Code" });
  } catch (error) {
