...
const previousDefaultRegExp = /^# nexus-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# nexus-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Nexus gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# nexus-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# nexus-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
